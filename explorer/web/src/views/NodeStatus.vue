    ...
    <div v-else class="status-container">
      ...
      <div class="status-card">
        ...
        <div class="info-grid">
          ...
          <div class="info-item">
            ...
            <span class="value">{{ status.blockchain?.memory_blocks || 0 }}</span>
          </div>
          <div class="info-item">
            <span class="label">체인 팁 해시:</span>
            <span class="value hash">{{ formatHash(status.blockchain?.chain_tip) }}</span>
          </div>
          <div class="info-item">
      ...
      <div class="status-card">
        ...
        <div class="info-grid">
          <div class="info-item highlight">
            ...
            <span class="value">{{ status.mempool?.pending_transactions || 0 }}</span>
          </div>
          <div class="info-item">
            <span class="label">확인된 트랜잭션:</span>
            <span class="value">{{ status.mempool?.seen_transactions || 0 }}</span>
          </div>
        </div>
      </div>
      ...
      <div class="status-card">
        ...
        <div class="info-grid">
          <div class="info-item highlight">
            ...
            <span class="value">{{ status.network?.connected_peers || 0 }}</span>
          </div>
        </div>
        ...
        <div v-if="peerHeights.length > 0" class="peer-list">
          ...
          <div class="peer-item" v-for="peer in peerHeights" :key="peer.id">
            ...
            <span class="peer-height">높이: {{ peer.height }}</span>
          </div>
        </div>
      ...
      <div class="auto-refresh-control">
        <label>
          ...
          자동 새로고침 ({{ refreshInterval / 1000 }}초마다)
        </label>
        <button @click="fetchStatus" class="refresh-btn">🔄 수동 새로고침</button>
      </div>
    </div>
  </div>
</template>
  ...
  setup() {
    ...
    const fetchStatus = async () => {
      try {
        ...
        error.value = err.response?.data?.message || '노드 상태를 가져올 수 없습니다'
      } finally {
        loading.value = false
      }
    }
